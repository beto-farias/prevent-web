...
function setupContentInfoWindow(dataO, marker) {
	...
	layout = '<div id="iw-container">' +
					...
					fileButton+
					'<div class="pull-right iw-subTitle">{fecha} - {distancia}</div>' +	
	...
	if(dataO.txt_referencia == null || dataO.txt_resumen.length==0){
		texto = "Sin descripción";
	}else{
		texto = dataO.txt_resumen;
	}
	
	if(dataO.txt_referencia == null || dataO.txt_referencia.length==0 || dataO.txt_referencia=="null"){
		referencia = " ";
	}else{
		referencia = "<a href='"+dataO.txt_referencia+"' target='_blank'>Ir a la nota<a/>";
	}
	
	botones = getHTMLInfoWindow(dataO);
	var distanciaFinal;
	if(distancia<1000){
		distanciaFinal = Math.round(distancia)+" Mts";
	}else{
		distanciaFinal = Math.round(distancia/1000)+" Km";
	}
	
	layout = layout.replace("{distancia}", distanciaFinal);
...
function getHTMLInfoWindow(dataO){
	var boton = "";
	var footer ="<div class='row' style='margin-left:0px; margin-right:0px'><div class='col-md-4'><img src='/images/resources/p_icon_criminal_sm.png'/><label class='labelCrimiVicti'>"+dataO.num_delincuentes+"</label></div>";
	footer+="<div class='col-md-4'><img src='/images/resources/p_icon_victima_sm.png'/><label class='labelCrimiVicti'>"+dataO.num_victimas+"</label></div>";
	
	if(!dataO.isHisDelito){
		footer+="<div class='col-md-4'><img src='/images/resources/p_icon_like_hand_sm.png' onClick='notGivePoint();' class='dgom-js-punto-delito' /><label class='labelCrimiVicti'>"+dataO.num_likes+"</label></div>";
	}else if(dataO.isHisDelito==-1){
		footer+="<div class='col-md-4'><img src='/images/resources/p_icon_like_hand_sm.png' onClick='givedPoint();' class='dgom-js-punto-delito' /><label class='labelCrimiVicti'>"+dataO.num_likes+"</label></div>";
	}else{
		footer+="<div class='col-md-4'><img src='/images/resources/p_icon_like_hand_sm.png' onClick='givePoint($(this));' class='dgom-js-punto-delito' data-delito='"+dataO.id_num_delito+"' data-evento='"+dataO.id_evento+"'/><label class='labelCrimiVicti'>"+dataO.num_likes+"</label></div>";
	}
...
function getIconMarker(type) {
	...
	//var puntoFin = new google.maps.Point(22, 52);
	var puntoFin = new google.maps.Point(22, 54);
...
function loadTipoSubtipo(idTipoDelito, subTipoDelito, isChecked){
	...
	if(isChecked){
		...
		if(delitosSubDelitos[idTipoDelito.toString()+subTipoDelito.toString()] && delitosSubDelitos[idTipoDelito.toString()+subTipoDelito.toString()].getMarkerCount(map.getZoom())){
